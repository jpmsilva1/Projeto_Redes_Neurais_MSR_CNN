...
class MSRCNNAttention1D(nn.Module):
    """
    MSR-CNN com mecanismo de Atenção entre Subbandas.
    Adiciona pesos dinâmicos para cada subbanda (Ruído, Sazonalidade, Tendência)
    dependendo da representação aprendida, antes da classificação final.
    """
    def __init__(self, in_channels, seq_len, num_classes=3):
        super(MSRCNNAttention1D, self).__init__()
        self.asd = AsymmetricASD1D(in_channels=in_channels, filter_size=5)
        ...
        # O mecanismo de atenção vai olhar para a concatenação e dar 3 pesos (um para cada subbanda)
        self.attention_fc = nn.Sequential(
            nn.Linear(cnn_out_features * 3, 64),
            nn.Tanh(),
            nn.Linear(64, 3),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        ...
        combined_raw = torch.cat([feat_noise, feat_seasonality, feat_trend], dim=1)
        
        # Calcula os pesos de atenção para cada subbanda [batch_size, 3]
        attn_weights = self.attention_fc(combined_raw)
        
        # Pondera as features de acordo com a atenção
        feat_noise_weighted = feat_noise * attn_weights[:, 0:1]
        feat_seasonality_weighted = feat_seasonality * attn_weights[:, 1:2]
        feat_trend_weighted = feat_trend * attn_weights[:, 2:3]
        
        combined_weighted = torch.cat([feat_noise_weighted, feat_seasonality_weighted, feat_trend_weighted], dim=1)
        
        out = self.fc(combined_weighted)
        
        # Retornamos os pesos para análise de interpretabilidade
